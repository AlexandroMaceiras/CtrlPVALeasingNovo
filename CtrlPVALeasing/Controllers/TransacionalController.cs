...
namespace CtrlPVALeasing.Controllers
{
    public class TransacionalController : Controller
    {
        ...
        public ActionResult Liquidados(DateTime? dataInicio, DateTime? dataFim, string uf_cliente, string impresso, string DUT, string criterio)
        {
            if (dataInicio == null)
                dataInicio = null;
            if (dataFim == null)
            ...
            model = (uf_cliente.Trim() == "SP" ? model.Where(x => x.uf_cliente == uf_cliente) : model.Where(x => x.uf_cliente != uf_cliente));
